pipeline {
    ...
    stages {
        ...
        steps {
            script {
            def repoUrl = 'https://github.com/rezaadit12/order-service.git'
        ...
        steps {
            ...
            git remote set-url origin https://${GITHUB_CREDENTIALS_USR}:${GITHUB_CREDENTIALS_PSW}@github.com/rezaadit12/order-service.git
        ...
        steps {
            script {
            def targetDir = "/home/reza/mini-soccer-project/order-service"
            ...
            ssh -o StrictHostKeyChecking=no -i ${SSH_KEY} ${USERNAME}@${HOST} '
                if [ -d "${targetDir}/.git" ]; then
                    ...
                    git clone -b "${TARGET_BRANCH}" git@github.com:rezaadit12/order-service.git "${targetDir}"
                    cd "${targetDir}"
                fi
